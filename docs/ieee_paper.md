...
A non-obvious engineering finding emerged during deployment: a `const` variable declaration in the main React component was being reordered by the esbuild version inside the Docker build environment relative to its first use in a React hook dependency array, causing a JavaScript Temporal Dead Zone (TDZ) `ReferenceError` visible only in production — not in local Vite builds. The two build environments used different optimization strategies for the same source code. The fix involved switching the affected binding to `var` (which hoists to function scope and has no TDZ regardless of ordering) and eliminating the binding from dependency arrays entirely, replacing it with direct state variable accesses. This underscores a broader risk in projects where local and CI/CD minifier versions diverge: correctness guarantees in development do not automatically transfer to deployment.

The admin knowledge dashboard state is persisted in `sessionStorage` rather than React state, so a page refresh restores the user's position instead of resetting to the default chat view. This is a small but meaningful UX detail for admin workflows that involve reading through long lists of knowledge gaps or crawl logs.
