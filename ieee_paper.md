...
React 18 with concurrent rendering, lazy-loaded heavy components (admin dashboard, eligibility checker, breathing companion, sign-out dialog), and error boundaries. All user interactions that trigger lazy component loading are wrapped in `startTransition` to prevent React error #426 (component suspended during synchronous input). A production-specific Temporal Dead Zone (TDZ) defect was identified and resolved: esbuild in the Docker build environment reordered a `const` declaration in the main component relative to its use in a hook dependency array, causing a `ReferenceError` that was absent in local builds. The fix uses `var` (hoisted, no TDZ) for the affected binding and replaces dependency array references with raw state variable accesses. Admin dashboard open state is persisted in `sessionStorage` so page refreshes restore the panel rather than dropping the user back to the chat. Vite 5 with Rollup bundling. Tailwind CSS with four theme variants (one per persona domain). Radix UI primitives for accessible modals, tooltips, dropdowns. PWA with service worker (cache-first assets, stale-while-revalidate app shell) and manifest shortcuts for direct persona launch.
...
- Build environment divergence between local Vite and Docker-based CI produced a TDZ defect visible only in production, highlighting a general risk in projects where the local and deployment minifiers differ in optimization strategy.
